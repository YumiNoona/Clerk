...
| `PlayerController` | Assign Character Controller and Main Camera; legacy Input Action fields may remain assigned as fallback |
...
The existing `UI Canvas` remains for authored world/gameplay UI:

| Component/object | Purpose |
|---|---|
| `UIController` | Shelf price editing panel |
| `InteractionPromptDisplay` | Crosshair interaction prompts |
| `WalletController` | Authoritative wallet adapter and legacy money text |
| `EventSystem` + `InputSystemUIInputModule` | Mouse, keyboard, and controller UI input |
...
| `WalletController` | Wallet balance and compatibility UI events |
| `UIController` | Authored shelf-price panel |
| `UIFactory` | Shared programmatic uGUI construction and visual theme |
| `StoreUIService` | Main menu, HUD, pause, desktop/mobile apps, settings, and saves |
